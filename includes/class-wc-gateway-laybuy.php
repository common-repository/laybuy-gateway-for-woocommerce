...
class WC_Payment_Gateway_Laybuy extends WC_Payment_Gateway
{
    ...
    public function __construct() {
        $logoUrl = plugins_url( 'assets/images/laybuy_badge_neon_grape-small.svg', __DIR__ );

        $this->id = self::GATEWAY_NAME;
        $this->icon = apply_filters( 'woocommerce_laybuy_gateway_icon', $logoUrl );
    ...
    public function shortcode_laybuy_logo($atts) {
        $atts = shortcode_atts( array(
            'width' => '65px',
            'link' => '',
            'style' => 'margin: 0;float:none; display:inline-block; vertical-align:middle; height:1.5em; top: -2px; position: relative;'
        ), $atts );

        $logoUrl = plugins_url( 'assets/images/', __DIR__ );

        if ($this->settings['laybuy_logo_theme'] === self::LOGO_THEME_WHITE) {
            $logoUrl .= 'laybuy_badge_neon_grape.svg';
        } else {
            $logoUrl .= 'laybuy_badge_neon_black.svg';
        }
        ...
        if (!empty($link)) {
            return sprintf(
                '<a href="%s" target="_blank" class="laybuy-logo-link"><img width="%s" style="%s" src="%s" alt="Laybuy" /></a>',
                $link,
                esc_attr($atts['width']),
                esc_attr($atts['style']),
                esc_url($logoUrl)
            ...
            return sprintf(
                '<img width="%s" style="%s" src="%s" alt="Laybuy" />',
                esc_attr($atts['width']),
                esc_attr($atts['style']),
                esc_url($logoUrl)
            );
